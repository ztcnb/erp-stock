<template>
  <el-card shadow="never">
    <div class="query-bar">
      <el-input v-model="query.keyword" placeholder="单号/供应商" clearable style="width: 200px" @keyup.enter="load(1)" />
      ...
      <el-button type="primary" :icon="Plus" @click="router.push('/purchase/edit')">新建采购单</el-button>
    </div>

    <el-table v-loading="loading" :data="rows" stripe>
      <el-table-column prop="orderNo" label="单号" width="150">
        <template #default="{ row }">
          <el-link type="primary" @click="router.push(`/purchase/edit/${row.id}`)">{{ row.orderNo }}</el-link>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          <template v-if="row.status === 'DRAFT'">
            <el-button link type="primary" @click="router.push(`/purchase/edit/${row.id}`)">编辑</el-button>
          ...
          <template v-else-if="row.status === 'APPROVED'">
            <el-button link type="success" @click="onInbound(row)">入库</el-button>
          </template>
...
async function load(page?: number) {
  ...
  loading.value = true
  try {
    const data = await purchaseApi.page({
      ...query,
...
async function onApprove(row: Row) {
  await ElMessageBox.confirm(`确定审核采购单 ${row.orderNo} 吗?审核后不可再编辑。`, '审核确认', { type: 'warning' })
...
async function onCancel(row: Row) {
  await ElMessageBox.confirm(`确定作废采购单 ${row.orderNo} 吗?`, '作废确认', { type: 'warning' })
  await purchaseApi.cancel(row.id)
  ElMessage.success('已作废')
  load()
}

async function onInbound(row: Row) {
  await ElMessageBox.confirm(
    `确定对采购单 ${row.orderNo} 执行入库吗?入库将按加权平均法更新库存成本,并生成应付账款。`,
    '入库确认',
    { type: 'warning' },
  )
